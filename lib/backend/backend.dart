...
import 'schema/closet_record.dart';
...
export 'schema/closet_record.dart';
...
/// Functions to query ClosetRecords (as a Stream and as a Future).
Future<int> queryClosetRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      ClosetRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<ClosetRecord>> queryClosetRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      ClosetRecord.collection,
      ClosetRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<ClosetRecord>> queryClosetRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<ClosetRecord>> queryClosetRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, ClosetRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      ClosetRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<ClosetRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
